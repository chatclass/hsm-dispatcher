...
function formatInativityDaysIds(inactivityDays: number[]){
  return inactivityDays.map((id: number, index: number) => { return index + 1 === inactivityDays.length ? `${id}` : `${id},`})
}
...
,not_onboarded as 
  (
    ...
    and   uc.course_id in (${formatCourseIds(courseIds)})
  )
 
 ,result as (select  whatsapp_id, course_id, instance_name from business_days where thediff in  (${formatInativityDaysIds(inactivityDays)})
   union 
